...
Ektron.PFWidgets.ContentBlock = {
    ...
    setupAll: function (widgetId) {
        ...
        // Hide add content/form buttons by default
    ...
    Tabs:
    {
        init: function () {
            ...
            // Check user permission on root folder to show/hide add content/form buttons
...
Ektron.PFWidgets.ContentBlock.Folder = {
    ...
    openToSelectedContent: function () {
        ...
        for (var i = fid.length; i >= 0; i--) {
            var clicktarget = null;
            if (i !== 0) {
                clicktarget = $ektron("div.CBfoldercontainer span.folder[data-ektron-folid='" + fid[i] + "']").parent().children("div.expandable-hitarea");
            }
            else {
                clicktarget = $ektron("div.CBfoldercontainer span.folder[data-ektron-folid='" + fid[i] + "']");
            }
        ...
        //now scroll to the folder
        $ektron(".CBEdit #ByFolder").scrollTo("div.CBfoldercontainer span.folder[data-ektron-folid='" + fid[0] + "']");
    ...
    configClickAction: function () {
        $ektron("div.CBfoldercontainer span[data-ektron-folid]").unbind("click").click(function () {
            ...
            // Hide add content/form buttons if trying to create under an invalid folder type or user does not have permission
...
function getRootPermission() {
    // summary:
    //      Get user permission on root folder

    $ektron.ajax({
        async: false,
        url: '/widgets/contentblock/cbhandler.ashx?action=checkRoot',
...
function checkRootPermissions() {
    // summary:
    //      Show or hide add content/form buttons depending on user permission

    if (firstRun) {
        firstRun = false;
        return getRootPermission();
    }

    var $CBTypeFilter = $ektron('.CBTypeFilter');
    var $CBAdd = $ektron(".CBAdd");
    var $CBAddForm = $ektron(".CBAddForm");

    if (permissions) {
        $CBTypeFilter.val() === "content" ? $CBAdd.show() : $CBAddForm.show();
    }
    else {
        $CBTypeFilter.val() === "content" ? $CBAdd.hide() : $CBAddForm.hide();
    }
