...
public partial class loginMin : System.Web.UI.Page
{
    ...
    private void Page_Load(System.Object sender, System.EventArgs e)
    {
        try
        {
            ...
            Session["RedirectLnk"] = "~/cmslogin.aspx";
    ...
    private bool CheckForResetRequest()
    {
        var result = false;

        try
        {
            if (IsPostBack && !string.IsNullOrWhiteSpace(LoginHiddenfield.Value)
                && LoginHiddenfield.Value == "request_reset")
            {
                result = true;
                LoginHiddenfield.Value = string.Empty;

                if (string.IsNullOrEmpty(emailAddressInput.Value) || string.IsNullOrWhiteSpace(emailAddressInput.Value))
                {
                    throw new Exception(m_refMsg.GetMessage("alert msg email required"));
                }

                m_refUserApi.EmailUserPasswordResetLink(emailAddressInput.Value);
                ResetLinkSentPanel.Visible = true;
                LoginRequestPanel.Visible = false;
            }
        }
        catch (Exception ex)
    ...
    protected void Page_PreInit(object sender, System.EventArgs e)
    {
        ...
        loginButton.InnerText = m_refMsg.GetMessageForLanguage("generic login msg", iSiteLanguage);
        loginButtonIE8.InnerText = m_refMsg.GetMessageForLanguage("generic login msg", iSiteLanguage);
        ...
        changePasswordText.InnerText = m_refMsg.GetMessage("lbl change password");
        changePasswordTextIE8.InnerText = m_refMsg.GetMessage("lbl change password");

        // forgot password text strings:
        ShowForgotPasswordBtnText.Text = m_refMsg.GetMessage("lbl Forgot Password");
        RequestResetPasswordBtnLbl.Text = m_refMsg.GetMessage("lbl Send Reset Password Link");
        CancelRequestResetPasswordBtnLbl.Text = m_refMsg.GetMessage("generic cancel");
        emailAddressInputLbl.InnerText = m_refMsg.GetMessage("generic email");
        ResetLinkSentNotification.Text = m_refMsg.GetMessage("msg password reset link has been sent");
        ResetLinkSentNotificationComplete.Text = m_refMsg.GetMessage("lbl click here to continue");
        ForgotPasswordUIErrorMessage.Text = m_refMsg.GetMessage("alert msg email required");
    }
            ...
            else
            {
                ...
                if (!pwdReset)
                {
                    ...
                    if (UserInfo == null || UserInfo.Id == 0)
                    {
                        ...
                        AuthenticationFailed(m_refMsg.GetMessage("username and password combination is not correct"));
                        return;
                    }
    ...
    private void ShowPasswordReset(UserData cmsUser, string oldPassword, bool IsInGracePeriod)
    {
        ...
        skipText.InnerText = m_refMsg.GetMessage("lbl skip change password");
        skipTextIE8.InnerText = m_refMsg.GetMessage("lbl skip change password");
